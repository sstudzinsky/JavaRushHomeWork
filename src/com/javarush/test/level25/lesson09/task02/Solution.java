...
public class Solution extends TimerTask {
    ...
    public Solution(TimerTask original) {
        ...
        this.original = original;   //init handler here

        class UEH implements Thread.UncaughtExceptionHandler{
            @Override
            public void uncaughtException(Thread t, Throwable e) {
                String newError = e.getMessage().replace(t.getName(), t.getName().replaceAll(".", "*"));

                e = new Exception(newError, e);
                System.out.println(e.getMessage());
            }
        }

        this.handler = new UEH();
    }
